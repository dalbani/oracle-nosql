...
public abstract class AbstractStoreExport implements CredentialsProvider {
     ...
     */
    private final RecordStreamMap recordStream;
    ...
    public AbstractStoreExport(String storeName,
        ...
        recordStream = new RecordStreamMap();
    ...
    public void exportTable(String[] tableNames) {
        ...
        for (String tableName : tableNames) {
            ...
            try {
                ...
                while (tableIterator.hasNext()) {
                     ...
                     * stream
                     */
                    recordStream.canAddRecord(tableName, recordLength);

                    /*
                     * Stream the table data bytes to the external export store.
                     */
                    recordStream.insert(tableName, keyLengthBytes);
                    recordStream.insert(tableName, keyBytes);
                    recordStream.insert(tableName, valueLengthBytes);
                    recordStream.insert(tableName, valueBytes);
                    if (expiryTimeInHours > 0) {
                        recordStream.insert(tableName, expiryBufLengthBytes);
                        recordStream.insert(tableName, expiryTimePacked);
                    } else {
                        recordStream.insert(tableName,
                                            zeroExpiryTimeLengthBytes);
                    }
                    recordStream.insert(tableName, checksumLengthBytes);
                    recordStream.insert(tableName, checkSumPacked);
        ...
        recordStream.performPostExportWork();
        doTableMetadataDiff();
    ...
    private void exportTableSchemas(String[] tableNames) {
        ...
        for (String tableName : tableNames) {
             ...
             * stream
             */
            recordStream.canAddRecord(schemaFileName, tableSchemaBytes.length);

            /*
             * Stream the schema bytes to the external export store
             */
            recordStream.insert(schemaFileName, tableSchemaBytes);
        }
    }
    ...
    public void export() {
        ...
        try {
            while (iter.hasNext()) {
                 ...
                 */
                if (value.getFormat().equals(Value.Format.TABLE)) {
    ...
    private void exportAvroSchemas() {
        ...
        for (Map.Entry<String, SchemaData> entry :
             ...
             * Check if the schema bytes can be transferred to RecordStream
             */
            recordStream.canAddRecord(schemaFileName, avroSchemaBytes.length);

            /*
             * Stream the schema bytes to the external export store
             */
            recordStream.insert(schemaFileName, avroSchemaBytes);
        }
    }
    ...
    private void exportAvroRecord(Key key, Value value) {
         ...
         */
        recordStream.canAddRecord(otherDataFile, recordLength);

        /*
         * Stream the avro record bytes to the external export store.
         */
        recordStream.insert(otherDataFile, identifierBytes);
        recordStream.insert(otherDataFile, schemaStringBytes);
        recordStream.insert(otherDataFile, keyLengthBytes);
        recordStream.insert(otherDataFile, keyBytes);
        recordStream.insert(otherDataFile, valueLengthBytes);
        recordStream.insert(otherDataFile, valueBytes);
        recordStream.insert(otherDataFile, checksumLengthBytes);
        recordStream.insert(otherDataFile, checkSumPacked);
    }
    ...
    private int exportTableRecord(Key key,
        ...
        if (expiryTimeInHours > 0) {
            ...
            recordStream.insert(tableName, zeroExpiryTimeLengthBytes);
        }
        recordStream.insert(tableName, checksumLengthBytes);
        recordStream.insert(tableName, checkSumPacked);
    ...
    class RecordStreamMap {
         ...
         */
        private final ConcurrentHashMap<String, RecordStream> map;
        ...
        public RecordStreamMap() {
            map = new ConcurrentHashMap<String, RecordStream>();
        ...
        void canAddRecord(String fileName, long recordLength) {
            ...
            RecordStream stream = map.get(fileName);
            ...
            /*
             * check if adding this record will exceed the file segment size
             * limit (1GB).
             */
            if (stream.getFileSize() + recordLength > maxFileSegmentSize) {

                logger.info("Chunk size for file " + fileName + " exceeded." +
                            " Flushing the stream.");
        ...
        void insert(String fileName, byte[] recordBytes) {
             ...
             * the record bytes in a separate thread.
             */
            if (stream == null) {
                ...
                stream = new RecordStream(fileName, out);
                map.put(fileName, stream);
            }

            stream.addRecord(recordBytes);
        }
        ...
        void insert(String lobName, ByteBuffer buffer) {
            ...
            RecordStream stream = map.get(lobName);
         ...
         * Flush the stream
         */
        public void flushStream(RecordStream stream) {

            CustomOutputStream out = stream.out;
            ...
            try {
                ...
                logger.log(Level.SEVERE, "Exception populating TaskWaiterQueue "
        ...
        public void flushAllStreams() {
            ...
            for (Map.Entry<String, RecordStream> entry :
                    map.entrySet()) {

                RecordStream stream = entry.getValue();
                flushStream(stream);
            }
    ...
    private class WriteLobTask implements Callable<Boolean> {
        ...
        public Boolean call() throws Exception {
            ...
            while (inChannel.read(buffer) >= 0 || buffer.position() > 0) {
                buffer.flip();
                recordStream.insert(lobName, buffer);
                buffer.compact();
            }
    ...
    /**
     * Worker thread responsible for reading the bytes from RecordStream and
