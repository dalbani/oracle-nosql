...
public abstract class Expr {
    ...
    static enum ExprKind {
        ...
        SLICE_STEP,   /* see class ExprSliceStep */
        FILTER_STEP,  /* see class ExprFilterStep */
    ...
    public final ExprIter getChildren() {
        ...
        if (theChildren.isOpen()) {
            throw new QueryStateException(
                "Attempt to reuse an already open ExprIter");
        }
    ...
    static class ExprIter implements Iterator<Expr> {
        ...
        public Expr next() {
            ...
            case SFW:
                ...
                if (theCurrentChild == 0) {
                    return sfw.getFromExpr();
                }

                int currentChild = theCurrentChild - 1;
            ...
            case FILTER_STEP: {
                ExprFilterStep step = (ExprFilterStep)theExpr;
                if (theCurrentChild == 0) {
                    return step.getInput();
                }
                assert(theCurrentChild == 1);
            ...
            case SLICE_STEP:
                ExprSliceStep step = (ExprSliceStep)theExpr;
                if (theCurrentChild == 0) {
                    return step.getInput();
                }
           ...
           case ARRAY_CONSTR:
            ...
            default:
                throw new QueryStateException(
                    "Unexpected expression type: " + theExpr.getKind());
            }
        }
        ...
        void remove(boolean destroy) {
            ...
            case SFW:
                ...
                if (theCurrentChild == 0) {
                    throw new QueryStateException("Cannot remove FROM clause");
                }
                ...
                if (!done && currentChild < sfw.getNumSortExprs()) {
            ...
            case FILTER_STEP: {
                ...
                if (theCurrentChild == 0) {
                    throw new QueryStateException(
                        "Cannot remove input expr from filter step");
                }

                assert(theCurrentChild == 1);
                assert(step.getPredExpr() != null);
                throw new QueryStateException(
                    "Cannot remove predicate expr from filter step");
            }
            case SLICE_STEP: {
                ExprSliceStep step = (ExprSliceStep)theExpr;
            ...
            case BASE_TABLE: {
               assert(theCurrentChild == 0);
               ((ExprBaseTable)theExpr).removeFilteringPred(destroy);
               --theNumChildren;
               --theCurrentChild;
               break;
            }
        ...
        void replace(Expr newChild, boolean destroy) {
            ...
            case SFW:
                ...
                if (theCurrentChild == 0) {
                    sfw.setFromExpr(newChild, destroy);
                    break;
                }
            ...
            case BASE_TABLE: {
               assert(theCurrentChild == 0);
               ((ExprBaseTable)theExpr).setFilteringPred(newChild, destroy);
               break;
            }
            case VAR:
            case CONST:
    ...
    final boolean isStepExpr() {
        ...
        if (theKind == ExprKind.FILTER_STEP ||
            theKind == ExprKind.SLICE_STEP ||
            theKind == ExprKind.FIELD_STEP) {
            return true;
        }

        Function func = getFunction(FuncCode.FN_KEYS);
