 ...
 * record.
 *
 * TODO ???? Replace the Map field with an array?
 */
...
public class RecordValueImpl extends ComplexValueImpl implements RecordValue {
    ...
    public int compareTo(FieldValue other) {
        ...
        if (other instanceof RecordValueImpl) {
            ...
            return compare(otherImpl, getFieldNamesInternal());
        }
    ...
    public List<String> getFields() {
        return Collections.unmodifiableList(getFieldNamesInternal());
    }

    /*
     * NOTE: this method will return null if the given field name does not
     * belong to the record schema or if it has never been set. ????
     */
    @Override
    ...
    public FieldValue get(int fieldIndex) {
        return valueMap.get(getDefinition().getFieldName(fieldIndex));
    }
    ...
    public RecordValue putRecord(String name, Map<String, ?> map) {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                "The map value type doesn't match " +
                "the field definition of the named field.", cce);
        }
        putField(name, val);
        return this;
    }
            ...
            while ((t = jp.nextToken()) != JsonToken.END_OBJECT) {
                ...
                if (fieldName != null) {
                    ...
                    if (fme == null) {
                        if (exact) {
                            throw new IllegalArgumentException(
                                "Unexpected field in JSON input: " + fieldName);
                        }
                        /*
                    ...
                    case ANY_ATOMIC:
                        ...
                        throw new IllegalArgumentException(
                            "Cannot create a record item with an imprecise " +
                                "field type");
                    }
                    ++numFields;
                }
    ...
    public FieldValueImpl getFieldValue(String fieldName) {
        return (FieldValueImpl)valueMap.get(fieldName);
    }
    ...
    public JsonNode toJsonNode() {
         ...
         */
        for (String fieldName : getFieldNamesInternal()) {
    ...
    public FieldValueImpl getComplex(TablePath tableField) {
    ...
    /**
     * Internal method to copy a field without any validation. This is
     * used when copying from a known good object.
     */
    private void putFieldInternal(String name, FieldValue value) {
        valueMap.put(name, value);
    }
    ...
    public FieldValue put(String name, String value, FieldDef.Type type) {
        ...
        FieldValue val = FieldDefImpl.createValueFromString(value, newField);
        putField(name, val);
        return val;
    }
    ...
    void copyFields(RecordValueImpl from) {
        for (Map.Entry<String, FieldValue> entry : from.valueMap.entrySet()) {
            putFieldInternal(entry.getKey(), entry.getValue().clone());
        }
    }
     ...
     */
    int compare(RecordValueImpl other, List<String> fieldList) {
    ...
    /*
     * Make sure that the type of the value matches the expected type.
     * This method will not work if the expected type is one of the ANY*
     * types. That case must be handled by potential callers.
     */
    FieldDefImpl validateNameAndType(String name, FieldDef.Type type) {
        ...
        assert (((FieldDefImpl)ft).isPrecise());

        if (ft.getType() != type) {
            throw new IllegalArgumentException(
                "Incorrect type for field " +  name + ", type is " +
                type + ", expected " + ft.getType());
        }

        return (FieldDefImpl) ft;
    }

    FieldDefImpl validateNameAndValue(String name, FieldValue value) {

        FieldDefImpl ft = getFieldDef(name);
        if (ft == null) {
            throw new IllegalArgumentException(
                "No such field in record " +
                getDefinition().getName() + ": " + name);
        ...
        FieldDefImpl valueDef = (FieldDefImpl) value.getDefinition();
        if (!valueDef.isSubtype(ft)) {
            throw new IllegalArgumentException(
                "Incorrect type for field " +  name + ", type is \n" +
                valueDef.toJsonString() + ", expected \n" +
                ft.toJsonString());
        }
    ...
    private static void checkNumberType(
        String fieldName,
        NumberType expected,
        NumberType actual,
        JsonParser jp)
        ...
        if (actual != expected) {
            ...
            throw new IllegalArgumentException(
                ...
                jp.getText() + ". Expected " + expected + ", is " + actual);
        }
    }
