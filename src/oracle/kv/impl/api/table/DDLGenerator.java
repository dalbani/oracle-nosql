...
public class DDLGenerator {
    ...
    private void generateDDL() {
         ...
         */
        getAllFieldDDL(fieldMap, rejectKeyList);
     ...
     */
    private void getAllFieldDDL(FieldMap fieldMap, List<String> rejectKeyList) {

        List<String> allFields = fieldMap.getFieldOrder();
        int numFields = allFields.size();

        for (int i = 0; i < numFields; i++) {
            String fname = allFields.get(i);

            if (rejectKeyList.contains(fname)) {
                continue;
            }

            FieldMapEntry entry = fieldMap.getFieldMapEntry(fname);
            FieldDef field = entry.getField();
    ...
    private void getFieldDDL(FieldDef field, String fname) {
        ...
        switch (field.getType()) {
            ...
            case FLOAT:
                /*
                 * Generate the ddl for String, Binary, Boolean, Double and
                 * Float type fields
                 */
                getDDL(field, fname);
                break;
    ...
    private String getIndexDDL(Index index) {
        ...
        for (int i = 0; i < numFields; i++) {
            ...
            sb.append(TableImpl.translateToExternalField(field));
    ...
    /**
     * Generates the ddl for String, Binary, Boolean, Double and Float
     * type fields
     */
    ...
    private void getRecordDDL(RecordDef field, String fname) {
        ...
        if (fname == null) {
             ...
             */
            getAllFieldDDL(fieldMap, new ArrayList<String>());
