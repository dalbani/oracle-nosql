...
class AdminStores {
     ...
     * @param txn a transaction
     */
    void putMetadata(Metadata<? extends MetadataInfo> md, Transaction txn) {
        switch (md.getType()) {
        case TABLE:
            getTableMDStore().putTableMetadata(txn, (TableMetadata)md);
            return;
        case SECURITY:
            getSecurityMDStore().putSecurityMetadata(txn, (SecurityMetadata)md);
            return;
        case TOPOLOGY:
            break;
        }
